...
async def command_start_handler(message: types.Message, state: FSMContext) -> None:
    ...
    balance_left = await display_amount_left(message.from_user.id)
    await message.answer(f"👋Привет, {message.from_user.full_name}!\n 💰 Твой остаток: {balance_left}", reply_markup=start_keyboard())
...
async def get_back(message: types.Message, state: FSMContext):
    ...
    balance_left = await display_amount_left(message.from_user.id)
    await message.answer(f"💰 Твой остаток: {balance_left}\n 😊Что теперь, {message.from_user.full_name}?\nТы можешь посмотреть свои расходы, а также добавить новые!", reply_markup=start_keyboard())
#INCOME HANDLE
...
@dp.message(IncomeForm.pass_income_amount, F.text.isdigit() | F.text.contains('.'))
async def input_income_handler(message: types.Message, state: FSMContext):
    try:
        data = await state.update_data(income = int(float(message.text)))
        await state.clear()
        ...
        await message.answer(f"❌ Доход не был внесен. Введите целое число", reply_markup=cancel_keyboard())
...
@dp.message(ExpenseForm.pass_expense_amount, F.text.isdigit() | F.text.contains('.'))
async def input_income_handler(message: types.Message, state: FSMContext):
    try:
        data = await state.update_data(expense_amount = int(float(message.text)))
        await state.clear()
        ...
        await message.answer(f"❌ Расход не был внесен. Введите целое число", reply_markup=cancel_keyboard())
...
async def get_by_time_handler(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"📅 Введите дату начала периода в формате 'ГГГГ-ММ-ДД'", reply_markup=cancel_keyboard())
